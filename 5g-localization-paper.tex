...
Therefore, new positioning algorithms need to be developed for 5G network. In ~\cite{Costca1} ~\cite{Costa2}, the authors propose a two-stage extended Kalman filter (EKF) that is based on reference signal received power (RSRP) measurements. However, the proposed algorithm only works when there are 
